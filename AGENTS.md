...
- `ObjectBase.apply_contact_displacement` / `apply_contact_velocity` are the only writes into an object's motion; everything else the step records goes into the object's `Contact2D` sensor, `obj.contact` (`world/sensors/contact2d.py`, registered in `SensorFactory` as `contact2d`: `in_contact`, `partners`, `records`, `force`, timers; cleared and ticked by the env, filled by the solver) and is read straight from it (`obj.contact.in_contact`, `partners`, `reports`, `force`, `contact_time`, `air_time`, `started`, `ended`; the object keeps no `contact_*` mirror properties); `collision_flag` stays False for resolved contacts
...
- `contact2d.py`: `Contact2D` (`type: contact2d`, named like `lidar2d`), the contact bookkeeping of one object (`in_contact`, `partners`, `records`, `force`, `contact_time`/`air_time`, `started`/`ended`) written by the env (`clear`/`tick`) and the contact solver (`add`/`add_force`); every object gets one built in as `obj.contact`, a plain attribute like `obj.lidar` (the boolean reading is `obj.contact.in_contact`; the object has no `contact_*` mirror properties, and a sensor and a flag never share a name), one listed under `sensors` is also drawn (contact points and force lines scaled by `force_scale`)
...
│   ├── sensors/        # Sensor implementations (lidar2d, fmcw_lidar2d, contact2d)
