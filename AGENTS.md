...
- **Central palette**: every default color the library draws with is a constant in `irsim/config/palette.py` (Okabe-Ito based, colour-blind safe, grayscale-print safe: robots green `#009E73`, acker `#117733`, obstacles black, arrow `#F0E442`, FOV `#56B4E9`/`#0072B2`, lidar `#CC3311`, FMCW velocity colors, and the `draw_points` / `draw_trajectory` / `draw_box` / `draw_quiver` helper defaults, where a format string such as `'r--'` still wins); change the look library-wide there and mirror the hex defaults in `docs/source/yaml_config/configuration.md` (HTML tree + entries) and the Chinese catalog
