...
class TestFMCWLidar2D:
    def test_laser_scan_message_uses_shared_fields(self):
        """LaserScan should contain only the shared message fields."""
    ...
    def test_laser_scan_metadata_reconstructs_simulated_beam_angles(self):
        """ROS angle metadata must describe every ray generated by IR-SIM."""
        sensor = Lidar2D(
            state=np.zeros((3, 1)),
            angle_range=np.pi,
            number=4,
        ...
        msg = LaserScan.from_sensor(sensor)
        reconstructed = msg.angle_min + np.arange(msg.ranges.size) * msg.angle_increment

        np.testing.assert_allclose(reconstructed, sensor.angle_list)
        assert msg.angle_max == pytest.approx(sensor.angle_list[-1])
        assert msg.time_increment == 0.0
        assert msg.scan_time == pytest.approx(0.2)

    def test_single_beam_laser_scan_points_forward(self):
        """A one-ray scan has one unambiguous zero-angle measurement."""
        sensor = Lidar2D(state=np.zeros((3, 1)), angle_range=np.pi, number=1)

        msg = LaserScan.from_sensor(sensor)

        np.testing.assert_array_equal(sensor.angle_list, [0.0])
