...
def between_vector(line_left_vector, line_right_vector, line_vector):
    ...
    if (
        cross_product(line_left_vector, line_vector) <= 0
        and cross_product(line_right_vector, line_vector) >= 0
    ):
        return True
    else:
        return False
...
def vel_select(state_tuple, neighbor_list, vo_outside, vo_inside, factor=1.0):
    ...
    if len(vo_outside) != 0:
        ...
        return min(
            vo_inside,
            key=lambda v: penalty(state_tuple, neighbor_list, v, vel_des, factor),
        )
...
def penalty(state_tuple, neighbor_list, vel, vel_des, factor):
    ...
    for moving in neighbor_list:
        ...
        distance_temp = distance(
            [moving[0], moving[1]], [state_tuple[0], state_tuple[1]]
        )
        ...
        vel_trans = [
            2 * vel[0] - state_tuple[2] - moving[2],
            2 * vel[1] - state_tuple[3] - moving[3],
        ]
